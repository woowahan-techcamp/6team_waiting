...
class StoreMapViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        currentMapView = self.initMap(frame: self.mapView.bounds)
    ...
    func putMap(store: Store) {

        if let lat = CLLocationDegrees(store.storeLatitude), let long = CLLocationDegrees(store.storeLongitude) {

            let nPoint = NGeoPoint(longitude: Double(lat), latitude: Double(long))

            currentMapView?.setMapCenter(nPoint, atLevel: 11)
        }
    }
}

extension StoreMapViewCell {
